...
export function StudentMessages() {
  ...
  useEffect(() => {
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'messages' }, (payload: any) => {
        ...
        void (async () => {
          try {
            ...
            console.error('Realtime student update failed:', err)
          }
        })()
      })
    ...
    try {
      void channel.subscribe((status: any) => {
        ...
        console.log('[supabase] channel status (student):', status)
      })
      // eslint-disable-next-line no-console
      console.log('[supabase] channel object (student):', channel)
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error('[supabase] subscribe error (student):', err)
    }
  ...
  useEffect(() => {
    ...
    console.log('[StudentMessages] Setting up polling fallback for conv:', selectedConvId)
        ...
        .then((data) => {
          setConversations((prev) => {
            if (JSON.stringify(data) !== JSON.stringify(prev)) {
              console.log('[StudentMessages] Polling: conversations updated!')
              return data
            }
            return prev
          })
        })
        .catch((err) => console.error('[StudentMessages] Polling conversations refresh failed:', err))
        ...
        .then((data) => {
          setMessages((prev) => {
            if (data.length !== prev.length) {
              console.log('[StudentMessages] Polling: new messages detected!', prev.length, '->', data.length)
              return data
            }
            return prev
          })
        })
        .catch((err) => console.error('[StudentMessages] Polling messages refresh failed:', err))
    }, 3000)

    return () => {
      console.log('[StudentMessages] Cleaning up polling')
